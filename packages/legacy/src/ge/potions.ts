import { getLastBuy, getLastSell } from "../utils/wiki";
...
export async function checkPotionsArbs() {
  ...
  if (brew.profit > 0) {
    console.log(
      `Brew profit: ${brew.profit.toFixed(2)} | sell3: ${
        brew.sell3.price
      } | buy4: ${brew.buy4.price}`
    );
  }
  if (restore.profit > 0) {
    console.log(
      `Restore profit: ${restore.profit.toFixed(2)} | sell3: ${
        restore.sell3.price
      } | buy4: ${restore.buy4.price}`
    );
  }
  if (stamina.profit > 0) {
    console.log(
      `Stamina profit: ${stamina.profit.toFixed(2)} | sell3: ${
        stamina.sell3.price
      } | buy4: ${stamina.buy4.price}`
    );
  }
  if (prayer.profit > 0) {
    console.log(
      `Prayer profit: ${prayer.profit.toFixed(2)} | sell3: ${
        prayer.sell3.price
      } | buy4: ${prayer.buy4.price}`
    );
  }
}
